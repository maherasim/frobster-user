...
Future<BaseResponseModel> walletTopUp(Map req,{String? endPoint}) async {
  // Delay is for showing loader again after loader was hidden from payment gateway
  await 100.milliseconds.delay;
  appStore.setLoading(true);
  dev.log('----------- Payment Method Called -----------');
  try {
    var res = BaseResponseModel.fromJson(await handleResponse(
        await buildHttpResponse(endPoint?? 'wallet-top-up',
...
Future<List<TransactionRequestData>> getTransactionRequest(int id) async {
  appStore.setLoading(true);
  try {
...
Future<BaseResponseModel> updateBooking(Map request) async {
  dev.log('Booking Save Response: ${jsonEncode(request.toString())}');
...
//region Payment Api
Future<BaseResponseModel> savePayment(Map request,{String? endPoint}) async {
...
Future<List<PaymentSetting>> getPaymentGateways(
  ...
  try {
    final raw = await handleResponse(await buildHttpResponse(
        'payment-gateway-list$isAddWalletStatus',
        method: HttpMethodType.GET));
...
Future<void> savePostJob(Map<String,dynamic> request,{ List<File>? imageFiles}) async {
...
Future<PostJobDetailResponse> getPostJobDetail(Map request,{bool toggleLoader = true}) async {
  try {
    var res = PostJobDetailResponse.fromJson(await handleResponse(
        await buildHttpResponse('get-post-job-detail',
            request: request, method: HttpMethodType.POST)));
    if(toggleLoader) appStore.setLoading(false);
...
Future<PostJobDetailResponse> getEditPostJobDetail(num id) async {
  try {
    final response = await handleResponse(
        await buildHttpResponse(
          'post-job-request/$id/editjob',
          method: HttpMethodType.GET,
        ));
    var res = PostJobDetailResponse.fromJson(response["data"]);
