...
class _ReasonDialogState extends State<ReasonDialog> {
  ...
  void saveHoldClick() async {
    final startAt = widget.status.bookingDetail!.startAt == null ? null :  DateTime.parse(widget.status.bookingDetail!.startAt!) ;
    final now = DateTime.now();
    final durationDiff = startAt == null ? 0 :  now.difference(startAt).inSeconds;
    Map request = {
      CommonKeys.id: widget.status.bookingDetail!.id.validate(),
