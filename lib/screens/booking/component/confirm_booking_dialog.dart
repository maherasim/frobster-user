...
class _ConfirmBookingDialogState extends State<ConfirmBookingDialog> {
  ...
  Future<void> bookServices() async {
    ...
    Map request = {
      ...
      BookService.amount: widget.servicePrice,
      ...
      CommonKeys.id: '',
      CommonKeys.scheduleSlots: widget.timeSlots.map((slot) => slot.toJson()).toList(),
    ...
    if (!widget.data.serviceDetail!.isFreeService &&
        widget.data.taxes.validate().isNotEmpty) {request.putIfAbsent('tax', () => widget.data.taxes);
    }
    ...
    saveBooking(request).then((bookingDetailResponse) async {
      ...
      if (widget.data.serviceDetail != null && widget.data.serviceDetail!.isAdvancePayment && !widget.data.serviceDetail!.isFreeService) {
        finish(context);
