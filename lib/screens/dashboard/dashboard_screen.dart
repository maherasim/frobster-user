...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  void initState() {
    ...
    LiveStream().on(LIVESTREAM_FIREBASE, (value) {
      if (value == 3) {
        // Redirect to Chat tab when a chat notification is tapped
        currentIndex = 4;
        setState(() {});
      }
    });
      ...
      child: Scaffold(
        body: AnimatedOpacity(
          ...
          child: [
            ...
            Observer(
              builder: (context) => appStore.isLoggedIn ? BookingFragment() : SignInScreen(isFromDashboard: true),
            ),
            Observer(
              builder: (context) => appStore.isLoggedIn ? MyPostRequestListScreen(fromDashboard: true): SignInScreen(isFromDashboard: true),
            ),
            CategoryScreen(),
            Observer(
